...
import { proveriKuponKod, type ProveraKuponaResp } from "../servisi/kuponi";
...
export default function Korpa() {
  ...
  const [primenjen, setPrimenjen] = useState<PrimenaKupona | null>(null);
  ...
  const popustRsd = primenjen?.popust ?? 0;
  const zaPlacanjeRsd =
    primenjen?.total_nakon_popusta ?? Math.max(0, medjuzbirRsd - popustRsd);

  const onPrimeniKupon = async () => {
    setGreskaKoda(null);
    ...
    setApplLoading(true);
    try {
      const r = await proveriKuponKod(kod, medjuzbirRsd);

      if (!r.ok) {
        const msg =
          (r as Extract<ProveraKuponaResp, { ok: false }>).razlog ??
          "Nepostojeći kod.";
        setPrimenjen(null);
        setGreskaKoda(String(msg));
        return;
      }

      const { kupon, popust, total_nakon_popusta } = r;

      setPrimenjen({
        kod: kupon.kod,
        tip: kupon.tip,
        vrednost: kupon.vrednost,
        description: kupon.description ?? undefined,
        popust: Math.round(Number(popust) || 0),
        total_nakon_popusta: Math.round(
          Number(total_nakon_popusta) || medjuzbirRsd
        ),
      });
    } catch (e: any) {
      setPrimenjen(null);
      setGreskaKoda(e?.message || "Greška pri proveri kupona.");
    } finally {
              ...
              <div className="card shadow-sm">
                <div className="card-body">
                  <div className="mb-3">
                    ...
                    <div className="input-group">
                      <input
                        ...
                        placeholder="npr. BIG15"
                        value={kodInput}
                        onChange={(e) => {
                          setKodInput(e.target.value);
                          setGreskaKoda(null);
                        }}
                        disabled={!!primenjen || applLoading}
                      />
                      {!primenjen ? (
                        <button
                          className="btn btn-outline-primary"
                          onClick={onPrimeniKupon}
                          disabled={applLoading}
                        >
                          {applLoading ? "Provera..." : "Primeni"}
                        </button>
                      ) : (
                        <button
                          className="btn btn-outline-secondary"
                          onClick={onUkloniKupon}
                        >
                          Ukloni kupon
                        </button>
                      )}
                    ...
                    {primenjen && (
                      <div className="form-text">
                        Aktivno: <strong>{primenjen.kod}</strong>
                        {primenjen.description
                          ? ` — ${primenjen.description}`
                          : ""}
                      </div>
                    )}
                    {greskaKoda && (
                      <div className="text-danger small mt-1">{greskaKoda}</div>
                    )}
                  </div>
                  ...
                  <div className="form-text mt-2">
                    Proverite količine i kupon pre plaćanja.
                  </div>
                </div>
              </div>
            </div>
